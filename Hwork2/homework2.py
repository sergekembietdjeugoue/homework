def package_loader():
    ...
    while True:
        try:
            max_items = int(input("Enter the maximum number of items to be shippe"))
            if max_items <= 0:
    ...
    while item_count < max_items:
        try:
            ...
            if weight < min_item_weight or weight > max_item_weight:
                ...
                continue
            # Check if it fits in current package
    ...
    # Send last package if it has weight
    ...
    if package_weights:
        ...
        #Find the package with max unused capacity
        unused_list = [max_package_weight - w for w in package_weights]
        max_unused = max(unused_list)
        package_with_max_unused = unused_list.index(max_unused) + 1 # package number
